...
void WaveformFifo::commitNewData()
{
    ...
    if (bufferWriteIndex == bufferSize) {
        ...
        //std::cout << "WaveformFifo::commitNewData: copying 'overhanging' data to beginning of buffer." << EndOfLine;
...
bool WaveformFifo::extractGpuSpikeDataOneDataBlock(uint16_t* waveform, GpuWaveformAddress waveformAddress, bool firstTime) const
{
    ...
    for (int k = 0; k < maxSpikesPerDataBlock; ++k) {
        ...
        if (spikeId != SpikeIdNoSpike) {
            ...
            spikeFound = true;
            // std::cout << "found spike " << (int) spikeId << " at timestamp " << spikeTimeStamp << " in channel " << waveformAddress.waveformIndex << EndOfLine;
...
//    std::cout << reader << ": " << minDistanceNew - minDistanceOld << EndOfLine;
    freeWords.release(minDistanceNew - minDistanceOld);
//    std::cout << reader << ":freeWords.release(" << minDistanceNew - minDistanceOld << "); Waveform FIFO is " << percentFull() << "% full." << EndOfLine;
//    std::cout << "freeWords.available() = " << freeWords.available() << EndOfLine;
}
